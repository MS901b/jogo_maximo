...
function addRow(valorMax, valorVenc, valorDado1, valorDado2) {
    ...
    doc = document.createTextNode(valorDado1);
    td.appendChild(doc);
    row.appendChild(td);
    td = document.createElement("TD");
    doc = document.createTextNode(valorDado2);
    td.appendChild(doc);
    ...
    td = document.createElement("TD");
    doc = document.createTextNode(valorMax);
    td.appendChild(doc);
    row.appendChild(td);
    td = document.createElement("TD");
    doc = document.createTextNode(valorVenc);
    td.appendChild(doc);
    row.appendChild(td);
...
function fazJogadas(nJogadas, desafio) {
    ...
    for (var i = 1; i <= nJogadas; i++) {
        ...
        // Pega o valor m�ximos dos que foram selecionados para os Dados
        ...
        // Chama addRow de acordo com quantos dados est�o sendo usados (para parte2)
...
function ApagarJogadas() {
    ...
    // Uma solu��o mais simples substituindo innerHtml da tabela � possivel no Firefox. Mas n�o funciona no IE.
    var tbody = document.getElementById("tabelaDados").getElementsByTagName("TBODY")[0];
...
function jogarTabela() {
    ...
    // Mostra a anima��o da dado rodando (dice0.gif)
    $("dado1").setAttribute('src', 'imgs/dice0.gif');
    $("dado2").setAttribute('src', 'imgs/dice0.gif');

    // Carrega valores aleat�rios nos dados.
    ...
    // Baseado nos valores de timeout, chama as fun��es que ir�o mostrar o valor dos dados.
...
// Funcao chamada ao terminar a jogada chamada na p�gina de Introdu��o
function fimJogadaTabela() {
    // Habilita o bot�o de jogar
    $("botaoJogar").destrava();
